...
function EditableNode({ data, id }: NodeProps) {
  ...
  return (
    <div
      ...
      style={{
        background: data.background || '#8ef292',
        color: data.color || '#121421',
        border: data.border || '1px solid #121421',
        borderRadius: '8px',
        padding: '8px 12px',
        minWidth: '100px',
        textAlign: 'center',
...
export function MindmapEditor({ isOpen, onClose, initialTitle }: MindmapEditorProps) {
  ...
  // Initialize with a central node when opened
  useEffect(() => {
  ...
  const handleSave = useCallback(() => {
    ...
    setNodeIdCounter(1);
    onClose();
  }, [mindmapTitle, nodes, edges, addNote, onClose, setNodes, setEdges]);
